...
// if user didn't modifed "PORT" object then read value from "config.json"
if(PORT === 9001) {
    PORT = config.port;
}
...
function serverHandler(request, response) {
    ...
    try {
        ...
        var matched = false;
        ['/public/', '/public/RTCMultiConnection', '/public/getHTMLMediaElement', '/socket.io/', '/node_modules/canvas-designer/'].forEach(function(item) {
        ...
        try {
            if (fs.statSync(filename).isDirectory()) {
                ...
                } else if (filename.indexOf(resolveURL('/public/dashboard/')) !== -1) {
                    filename = filename.replace(resolveURL('/public/dashboard/'), '');
                    filename += resolveURL('/public/dashboard/index.html');
                } else if (filename.indexOf(resolveURL('/public/video-conference/')) !== -1) {
                    filename = filename.replace(resolveURL('/public/video-conference/'), '');
                    filename += resolveURL('/public/video-conference/index.html');
